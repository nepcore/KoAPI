...
package object dsl {
  ...
  implicit def routePart(part: RouteParam[_]): IndexedSeq[RoutePart] =
  ...
  implicit class FromRoutePart(part1: String) {
    ...
    def /(part2: RouteParam[_]): IndexedSeq[RoutePart] =
      IndexedSeq(SimpleRoutePart(part1), ParametrizedRoutePart(part2))
  }
  implicit class FromRouteParam(part1: RouteParam[_]) {
    def /(part2: String): IndexedSeq[RoutePart] =
